            ...
            for case_id in log.flat_log:
                if heuristic == 'all':
                    for state in event_states[a]:
                        if state not in log.flat_log[case_id]:
...
def T_filtered(log, T, nodes, meta_states, heuristic='all'):
    ...
    for case_id in log.flat_log:
        case_log = ['start'] + list(log.flat_log[case_id]) + ['end']     
        for k in range(1,len(case_log)-1):
            a_i = case_log[k-1]
            a_k = case_log[k]
            a_j = case_log[k+1]
...
def add_frq(nodes, nodes_to_add, meta_states, T, heuristic='all'):
    ...
    for v in nodes:
        if v in meta_states:
            nodes1[v] = (nodes[v][0],nodes[v][1],dict())
            for v_i in v:
                if v_i not in nodes1[v][2]:
                    nodes1[v][2][v_i] = nodes[v][0]
                if heuristic == 'all':
